...
- event clustering for source/topic/node intelligence, with event, cross-language, source-divergence, missing-coverage feed cards, cluster quality, language-bridge terms, coverage-gap automation tasks, source candidate drafts, source discovery/validation/onboarding history, and draft resolution provenance
...
curl http://localhost:8000/api/v1/intelligence/clusters/<cluster_id>/source-drafts \
  -H "X-Parallax-Admin-Key: <admin_api_key>"
curl -X POST http://localhost:8000/api/v1/intelligence/clusters/<cluster_id>/source-drafts/<candidate_id>/resolve \
  -H "X-Parallax-Admin-Key: <admin_api_key>" \
  -H "Content-Type: application/json" \
  -d '{"status":"ignored","resolution_notes":"No usable source found yet."}'
curl -X POST "http://localhost:8000/api/v1/sources/discover?include_external=true&limit=8" \
  -H "X-Parallax-Admin-Key: <admin_api_key>" \
  -H "Content-Type: application/json" \
  -d '{"query":"Spanish regional power grid coverage news rss","language":"Spanish","source_type":"newspaper"}'
curl -X POST "http://localhost:8000/api/v1/sources/discover/validate?limit=5" \
    -H "X-Parallax-Admin-Key: <admin_api_key>" \
    -H "Content-Type: application/json" \
    -d '{"candidate":{"name":"Example","website_url":"https://example.com","rss_url":"https://example.com/rss.xml"}}'
curl -X POST "http://localhost:8000/api/v1/sources/discover/onboard?sync_article_limit=5&sync_card_limit=5&analysis_article_limit=10" \
    -H "X-Parallax-Admin-Key: <admin_api_key>" \
    -H "Content-Type: application/json" \
    -d '{"candidate":{"name":"Example","website_url":"https://example.com","rss_url":"https://example.com/rss.xml"},"sync_after_create":true,"analyze_after_sync":true,"refresh_intelligence":true,"refresh_clusters":true}'
curl -X POST "http://localhost:8000/api/v1/sources/discover/onboard-batch?limit=6" \
    -H "X-Parallax-Admin-Key: <admin_api_key>" \
    -H "Content-Type: application/json" \
    -d '{"items":[{"candidate":{"name":"Example","website_url":"https://example.com","rss_url":"https://example.com/rss.xml"}}],"require_review_before_sync":true}'
curl http://localhost:8000/api/v1/sources/discovery-runs \
    -H "X-Parallax-Admin-Key: <admin_api_key>"
curl http://localhost:8000/api/v1/sources/validation-runs \
    -H "X-Parallax-Admin-Key: <admin_api_key>"
curl http://localhost:8000/api/v1/sources/onboarding-runs \
    -H "X-Parallax-Admin-Key: <admin_api_key>"
curl http://localhost:8000/api/v1/sources/onboarding-batches \
    -H "X-Parallax-Admin-Key: <admin_api_key>"
curl -X POST http://localhost:8000/api/v1/sources/onboarding-runs/<run_id>/retry \
    -H "X-Parallax-Admin-Key: <admin_api_key>" \
    -H "Content-Type: application/json" \
    -d '{"require_review_before_sync":true}'
...
`scripts/refresh_event_clusters.py` and `POST /api/v1/intelligence/clusters/refresh` rebuild deterministic event clusters from ingested article metadata, saved analysis, comparison hooks, entities, claims, frames, URLs, event fingerprints, and bounded cross-language alias terms. The cluster layer persists `event_clusters`, `event_cluster_articles`, `event_cluster_refresh_runs`, and `source_draft_resolutions`; compare responses include `event_cluster` when the selected article belongs to one; topic detail can request topic-matched clusters; feed cards surface `event_cluster`, `cross_language_cluster`, `source_divergence`, `coverage_gap`, and `missing_coverage` signals. Cluster provider metadata includes quality score, average similarity, strategy counts, source/language diversity, language bridge terms, coverage-gap tasks, suggested source searches, source candidate drafts, and recommended actions. `GET /api/v1/intelligence/clusters/<cluster_id>/source-drafts` returns admin-gated source-manager prefill drafts generated from those gaps, and `POST /api/v1/intelligence/clusters/<cluster_id>/source-drafts/<candidate_id>/resolve` records `draft`, `created`, `resolved`, or `ignored` workflow status. `POST /api/v1/sources/discover` converts a draft search query into source candidates from the default source database and, when `EXTERNAL_RETRIEVAL_ENABLED=true`, public web search results; candidates remain retrieval leads rather than endorsements and each request is persisted in `source_discovery_runs`. `POST /api/v1/sources/discover/validate` tests candidate RSS URLs and homepage fallback with the existing ingestion parsers, returns `validated`, `failed`, or `needs_review`, and stores `source_candidate_validation_runs`. `POST /api/v1/sources/discover/onboard` validates a candidate, creates or reuses the source/feed, optionally syncs, analyzes pending articles, refreshes source intelligence, refreshes event clusters, records coverage delta metrics, and writes `source_onboarding_runs` for source-manager timelines. `POST /api/v1/sources/discover/onboard-batch` processes multiple candidates, can enforce `require_review_before_sync`, writes `source_onboarding_batches`, and refreshes clusters once at the end. `POST /api/v1/sources/onboarding-runs/<run_id>/retry` replays a stored onboarding request with optional phase overrides. Creating a source with `draft_cluster_id` and `draft_candidate_id` links the new source back to the coverage gap. Clusters are retrieval context, not truth judgments.
...
- Event clustering operationalization complete: `event_clusters`, event-cluster feed cards, compare/topic cluster context, scheduler script, pipeline controls, cross-language alias matching, cluster quality metadata, language-bridge terms, coverage-gap tasks, suggested source searches, admin source candidate drafts, source discovery candidates, candidate validation, source onboarding runs/batches, retryable onboarding, review-gated ingestion, coverage delta summaries, and draft resolution provenance persist cross-source event groups.
