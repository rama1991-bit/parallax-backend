...
- source/topic intelligence aggregation snapshots for fast source and topic pages
...
*/30 * * * * cd /app && python scripts/refresh_intelligence_snapshots.py --source-limit 50 --topic-limit 50 --article-limit 100
```

For platforms with scheduled jobs, run ingestion every 15 minutes and refresh intelligence snapshots shortly after ingestion, usually every 15-60 minutes. For platforms that prefer HTTP jobs, call `POST /api/v1/sources/sync-active` with `X-Parallax-Admin-Key` and use the script for snapshot refresh until an HTTP batch refresh endpoint is needed. Keep the batch limits in place until the source database has been reviewed for feed quality and traffic behavior.
