...
- event clustering for source/topic/node intelligence, with event, cross-language, source-divergence, missing-coverage feed cards, cluster quality, language-bridge terms, coverage-gap automation tasks, source candidate drafts, and draft resolution provenance
...
- Event clustering operationalization complete: `event_clusters`, event-cluster feed cards, compare/topic cluster context, scheduler script, pipeline controls, cross-language alias matching, cluster quality metadata, language-bridge terms, coverage-gap tasks, suggested source searches, admin source candidate drafts, and draft resolution provenance persist cross-source event groups.
