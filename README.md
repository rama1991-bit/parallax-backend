...
- Event clustering operationalization complete: `event_clusters`, event-cluster feed cards, compare/topic cluster context, scheduler script, pipeline controls, cross-language alias matching, cluster quality metadata, language-bridge terms, coverage-gap tasks, suggested source searches, admin source candidate drafts, source discovery candidates, candidate validation, source onboarding runs, coverage delta summaries, and draft resolution provenance persist cross-source event groups.
