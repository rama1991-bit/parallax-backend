...
For platforms with scheduled jobs, run ingestion every 15 minutes and refresh intelligence snapshots and event clusters shortly after ingestion, usually every 15-60 minutes. For platforms that prefer HTTP jobs, call `POST /api/v1/sources/sync-active`, `POST /api/v1/intelligence/refresh`, and `POST /api/v1/intelligence/clusters/refresh` with `X-Parallax-Admin-Key`. Keep the batch limits in place until the source database has been reviewed for feed quality and traffic behavior.
